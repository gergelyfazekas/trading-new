...
class StockList():
    def __init__(self, tickers, *args, **kwargs):
        ...
        self.stocks = {ticker: Stock(ticker, *args, **kwargs) for ticker in self.tickers}
 
    # this gives your object the []-syntax
    def __getitem__(self, key):
        # you can add validation, type-checks, logging … here
        try:
            return self.stocks[key]
        except KeyError:
            raise KeyError(f"{key!r} is not a valid element name")

    # optional extras that make it behave even more like a dict/sequence
    def __setitem__(self, key, value):
        self.stocks[key] = value          # enable c['x'] = val

    def __delitem__(self, key):
        del self.stocks[key]              # enable del c['x']

    def __contains__(self, key):
        return key in self.stocks         # enable 'x' in c

    def __iter__(self):
        return iter(self.stocks)          # let you loop over keys

    def __len__(self):
        return len(self.stocks)           # len(c)

    def pull_data(self, start=START_DATE, end=END_DATE):
        all_data = yf.download(self.tickers, start=start, end=end)
        for ticker, stock in self.stocks.items():
            df = all_data.xs(ticker, axis=1, level=1)
            df.columns = [col.lower().strip() if col.lower().strip() != 'date' else 'dt' for col in df.columns]
    ...
    def save_data(self, path):
        for ticker, stock in self.stocks.items():
            stock.data.to_csv(path + '/' + stock.name + '.csv', index_label='dt')

    def load_data(self, path):
        for ticker, stock in self.stocks.items():
            df = pd.read_csv(path + '/' + ticker + '.csv').set_index('dt', drop=True).sort_index()
            stock.set_data(df)

    def clear_data(self):
        for ticker, stock in self.stocks.items():
            try:
                stock.data = stock.data.iloc[:0]
            except AttributeError:
                pass # before initialized: 'NoneType' object has no attribute 'iloc'
...
class Stock:
    ...
    def __init__(self, name, sector=None, start_date=START_DATE, end_date=END_DATE, data=None):
    ...
    def get_stocks_per_date(total_df, verbose=False):
        ...
        df1 = pd.DataFrame(total_df.groupby('dt')['ticker'].apply(lambda x: len(x)))
        df1.rename(columns={'ticker': 'len_ticker'}, inplace=True)
        df2 = pd.DataFrame(total_df.groupby('dt')['ticker'].apply(list))
    ...
    def set_index(self):
        if self.data.index.name != 'dt':
            if self.data.index.name != 'Date':
                if not 'dt' in self.data.columns:
                    try:
                        self.data['dt'] = self.data['Date']
                    except KeyError:
                        raise ('No "dt" or "Date" column')
                else:
        ...
        self.data['dt'] = self.data.index.copy()
    ...
    def set_sector(self):
        try:
            ...
            print('Sector not found. Assigning `Unknown`')
    ...
    def calc_return(self, mode="log"):
        ...
        if mode == "normal":
            ...
            self.data['log_return'] = tmp_df['log_return'].copy()
        else:
    ...
    def get_price(self, as_of, strict=False):
        """ returns the price of the stock at as_of. 
        strict=True --> if we don't have data for the as_of date then error out
        strict=False --> return the last price before as_of if we don't have as_of"""
        try:
            return self.data.loc[as_of, 'close']
        except KeyError:
            if self.last_date < as_of:
                if strict:
                    raise ValueError(f'No data found for {as_of}. Modify the data or use strict=False.')
                else:
                ...
                if not strict:
                    ...
                    return self.get_price(previous_day)
                else:
                    raise ValueError(f'No data found for {as_of}. Modify the data or use strict=False.')
        except TypeError:
    ...
    def get_high_volumes(self, from_date=datetime.date(2000, 1, 1), to_date=datetime.date.today(),
        ...
        peak_vol_dates = data_chunk['dt'][peak_vol_idx]
        ...
        peaks_df = data_chunk.loc[self.data['dt'][unique_dates.drop_duplicates()], ('dt', 'volume')]
        return peaks_df
    ...
    def get_tech_levels(self, from_date=datetime.date(2000, 1, 1), to_date=datetime.date.today(), **kwargs):
        ...
        high_price_df = high_price_df.loc[:, ('dt', 'close')]

        if consider_volume:
            unique_dates = pd.concat([high_vol_df['dt'], high_price_df['dt']], ignore_index=True)
            tech_df = data_chunk.loc[unique_dates.drop_duplicates(), ('dt', 'close')]
    ...
    def cross_validate(cls, total_df, label_str, skip_cols, method, init_ratio=0.99, **kwargs):
        ...
        for current_date in total_df['dt'].loc[init_date:].unique():
            print('current_date', current_date)
            # forecast date is the second element (1st index) of the unique upcoming days
            try:
                forecast_date = total_df['dt'].loc[current_date:].unique()[1]
            except IndexError:
                print('Done')
                return model
    ...
    def fit_predict(cls, total_df, label_str, predict_col_name, skip_cols, method, train_ratio=0.5, predict="train", **kwargs):
        ...
        # fitting
        model = method(training_slice, label_str, **kwargs)
        print('unique_dates', total_df['dt'].loc[init_date:].unique())
        for current_date in total_df['dt'].loc[init_date:].unique():
            print('current_date', current_date)
            # forecast date is the second element (1st index) of the unique upcoming days
            try:
